        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {filtered.map((v, i) => (
            <Link
              key={v.id}
              href={`/inventory/${v.id}`}
              className="animate-fade-up bg-surface border border-border rounded-xl overflow-hidden hover:border-accent/30 transition-all group"
              ...
              <div className="aspect-[4/3] relative bg-surface-2">
                ...
                <div className="absolute top-2 left-2">
                  {v.photo_status === 'processed' ? (
                    ...
                    <span className="text-xs px-2 py-1 rounded-md font-medium bg-accent/80 text-black">
                      Pending
                    </span>
                  )}
                </div>
                ...
                {v.dom > 0 && (
                  <div className={`absolute top-2 right-2 text-xs px-2 py-1 rounded-md font-medium ${
                    v.dom > 60 ? 'bg-red-500/80 text-white' : v.dom > 30 ? 'bg-accent/80 text-black' : 'bg-black/60 text-white'
                  }`}>
                    {v.dom}d
                  </div>
                )}
                ...
                {filtered.map((v) => (
                  <tr key={v.id} className="border-b border-border/50 hover:bg-surface-2 transition-colors">
                    <td className="px-4 py-3">
                      <Link href={`/inventory/${v.id}`} className="hover:text-accent transition-colors font-medium">
                        {v.heading}
                      </Link>
                    </td>
                    ...
                    <td className="px-4 py-3 text-right">
                      <span className={v.dom > 60 ? 'text-danger' : v.dom > 30 ? 'text-accent' : ''}>
                        {v.dom}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex gap-1">
                        {v.photo_status === 'processed' ? (
                          ...
                          <span className="text-xs bg-accent/15 text-accent px-1.5 py-0.5 rounded font-medium">Pending</span>
                        )}
